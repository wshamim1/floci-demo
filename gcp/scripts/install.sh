#!/usr/bin/env bash
# install.sh — Install all Floci GCP dependencies on macOS (arm64 / x86_64)
# Supports Docker Desktop or Podman as the container runtime.
# Usage: bash gcp/scripts/install.sh [--docker | --podman]
...
echo ""
echo -e "${CYAN}╔══════════════════════════════════════╗${NC}"
echo -e "${CYAN}║    Floci GCP Installer — macOS        ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════╝${NC}"
echo ""
...
# ── 3. Google Cloud SDK (gcloud + gsutil) ─────────────────────────────────────
if ! command -v gcloud &>/dev/null; then
  ...
  success "Google Cloud SDK already installed ($(gcloud --version 2>&1 | head -1))"
fi

# ── 4. Pull GCS emulator image ─────────────────────────────────────────────────
info "Pulling fake-gcs-server image (fsouza/fake-gcs-server:latest) with ${RUNTIME}..."
"${RUNTIME}" pull fsouza/fake-gcs-server:latest
success "fake-gcs-server:latest pulled."

# ── 5. Pull Pub/Sub emulator image ────────────────────────────────────────────
info "Pulling Pub/Sub emulator image (gcr.io/google.com/cloudsdktool/google-cloud-cli:emulators) with ${RUNTIME}..."
"${RUNTIME}" pull gcr.io/google.com/cloudsdktool/google-cloud-cli:emulators
success "google-cloud-cli:emulators pulled."

echo ""
echo -e "${GREEN}✔  All dependencies installed successfully (runtime: ${RUNTIME}).${NC}"
echo ""
echo "  Next steps:"
echo "    bash gcp/scripts/start.sh      # start GCS + Pub/Sub emulators"
echo "    bash gcp/scripts/test.sh       # run smoke tests"
echo ""
